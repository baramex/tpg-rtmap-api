...
pub async fn get_trip_shape(
    identifier: Path<TripIdentifier>,
    database: Data<Database>,
) -> Result<Json<ShapeResponse>, TripError> {
    let id: Result<i32, std::num::ParseIntError> = identifier.into_inner().id.parse::<i32>();
    if id.is_err() {
        return Err(TripError::BadTripRequest);
    }
    ...
    if trip.is_none() {
        return Err(TripError::TripNotFound);
    }
    ...
    let shape_points: Option<Vec<ShapePoint>> = database
        .get_many::<ShapePoint>(
            ...
            .bind(trip.as_ref().unwrap().shape_id),
        )
        .await;
    ...
    match shape_points {
        Some(shape_points) => Ok(Json(ShapeResponse {
            points: shape_points,
            stops: shape_stops.unwrap_or(Vec::new()),
        })),
        None => Err(TripError::TripNotFound),
    }
}
